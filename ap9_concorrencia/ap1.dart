...
// Função assíncrona que verifica o status de entrega chamando a API
Future<void> verificarStatusEntrega(String codigoRastreamento) async {
  // Aguarda o resultado da consultarAPI antes de prosseguir
  String status = await consultarAPI(codigoRastreamento);
  print('Status de entrega do pacote $codigoRastreamento: $status');
}

// Função principal assíncrona - demonstra execução sequencial com async/await
Future<void> main() async {
  print('Inicio');
  // Aguarda cada verificação antes de prosseguir para a próxima
